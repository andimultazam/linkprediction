...
def run_algo(masterTrainEdgeFile):
    ...
    # Compute similarity
    cnGraphs = cnGraphs.map(lambda cnGraph: calculateSimilarity(cnGraph, bc))
    ########## Done Compute Similarity #####################

    ##########Step 5: Compute suggestion for each node###############
    nodeSuggestions = cnGraphs.flatMap(lambda cnGraph: getNodeSuggestions(cnGraph))
...
def evaluateROC(nodeSuggetions, thresholds):
    ...
    for i in range(len(thresholds)):
        ...
        outFile = trainFolder + "/cncf_threshold=" + str(thresholds[i]) + ".result"
...
def evaluateNoOfCorrectPrediction(nodeSuggestions, K):
    ...
    for i in range(K):
        ...
        outFile = trainFolder + "/cncf_k=" + str(i+1) + ".result"
        Utils.removeExistingDir(outFile)
...
if __name__ == '__main__':
    ...
    thresholds = [1, 2, 3, 4, 5, 7, 10, 20, 30, 40]
    K = 10
